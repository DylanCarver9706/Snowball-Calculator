...
export default function PdfToPng() {
  ...
  useEffect(() => {
    const storedCount = localStorage.getItem("pdfToPngUsageCount");
    if (storedCount) {
  ...
  const incrementUsageCount = () => {
    ...
    localStorage.setItem("pdfToPngUsageCount", newCount.toString());
  };
  ...
  const downloadSinglePage = (page: ConvertedPage) => {
    saveAs(page.blob, `${file?.name.split(".")[0]}_page_${page.number}.png`);
  };
  ...
  const downloadAllPagesZip = async () => {
    ...
    convertedPages.forEach((page) => {
      zip.file(
        `${file?.name.split(".")[0]}_page_${page.number}.png`,
        page.blob
      );
    });
